...
<div class="relative flex-1 lg:ml-64 transition-all duration-300 p-6 bg-gray-100 top-0 bottom-0 border-l-8 border-[#ddd] lg:border-l-8 max-lg:border-l-0">
    ...
    <div class="flex justify-between items-center mb-8"> 
        <div class="mb-8">
            <h1 class="lg:text-2xl font-bold text-gray-800 -mt-5 pt-5 hidden md:block">Klaim Poin</h1>
        </div>
    ...
    <details class="day-activity bg-green-50 p-4 rounded-lg shadow-md transition duration-300 hover:shadow-lg open:bg-green-100 mb-3">
        <summary class="flex justify-between items-center font-medium text-green-800 cursor-pointer">
            <span class="text-green-800 font-medium">
                <i class="fas fa-calendar-day mr-2"></i>
                {{ $challenge->no_tantangan ?? 'Tantangan #' . $challenge->id }}
            </span>
            <div class="flex items-center">
                <span class="coin-box flex items-center mr-4">
                    🪙 {{ $challenge->poin_tantangan ?? 0 }}
                </span>

                {{-- Tombol Klaim --}}
                @if ($challenge->is_claimed)
                    <button type="button" class="bg-gray-400 text-white py-1 px-3 rounded-lg cursor-not-allowed" disabled>
                        ✔️ Diklaim
                    </button>
                @elseif ($challenge->challenge_status === 'submitted' || $challenge->submission_status === 'completed')
                    <form method="POST" action="{{ route('klaim.poin', $challenge->id) }}">
                        @csrf
                        <button type="submit" class="bg-[#578669] text-white py-1 px-5 rounded-lg hover:bg-green-600">
                            🎁 Klaim
                        </button>
                    </form>
                @else
                    <button type="button" class="bg-gray-400 text-white py-1 px-11 rounded-lg cursor-not-allowed" disabled>
                        🔒 Belum Selesai
                    </button>
                @endif
            </div>
        ...
        <h2 class="text-gray-700 font-semibold mt-3">{{ $challenge->title ?? 'Judul Tantangan' }}</h2>

        @php
            $tasks = isset($challenge->details) ? preg_split('/[\n,]/', $challenge->details) : [];
        @endphp
