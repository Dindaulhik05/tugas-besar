...
<div class="relative flex-1 lg:ml-64 transition-all duration-300 p-6 bg-gray-100 top-0 bottom-0 border-l-8 border-[#ddd] lg:border-l-8 max-lg:border-l-0">
    ...
    <div class="flex justify-between items-center mb-8"> 
        <div class="mb-8">
            <h1 class="lg:text-2xl font-bold text-gray-800 -mt-5 pt-5 hidden md:block">Kelola Hadiah</h1>
        </div>
    ...
    <div class="container mx-auto">
        <div class="flex justify-start mb-6">
            <a href="{{ route('hadiah.create') }}" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600"> + Tambah Hadiah</a>
        </div>
    ...
    @foreach($hadiahs as $hadiah)
        <div class="bg-white shadow-lg rounded-xl overflow-hidden flex flex-col">
            ...
            @if($hadiah->image)
                <img src="{{ asset('storage/' . $hadiah->image) }}" alt="{{ $hadiah->nama_hadiah }}" class="w-full h-40 object-contain">
            @else
            ...
            <div class="p-4 flex flex-col justify-between flex-grow">
                <div class="mb-4">
                    <p class="text-sm text-gray-600 mb-1">🎯 Point: <span class="font-semibold">{{ $hadiah->points }}</span></p>
                    <p class="text-sm text-gray-600">📦 Stok: <span class="font-semibold">{{ $hadiah->stok }}</span></p>
                </div>

                <div class="mt-auto flex space-x-2">
                    <a href="{{ route('hadiah.edit', $hadiah->id) }}" class="flex-1 text-center bg-blue-500 text-white py-2 rounded-md font-semibold hover:bg-blue-600 transition">Edit</a>

                    <form action="{{ route('hadiah.destroy', $hadiah->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Kamu yakin ingin menghapus data ini?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full bg-red-500 text-white py-2 rounded-md font-semibold hover:bg-red-600 transition">Delete</button>
                    </form>
                </div>
            </div>
        </div>
