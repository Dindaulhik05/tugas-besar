...
<div class="relative flex-1 lg:ml-64 transition-all duration-300 p-6 bg-gray-100 top-0 bottom-0 border-l-8 border-[#ddd] lg:border-l-8 max-lg:border-l-0">
    ...
    <div class="flex justify-between items-center mb-8"> 
        <div class="mb-8">
            <h1 class="lg:text-2xl font-bold text-gray-800 -mt-5 pt-5 hidden md:block">Tambah Hadiah</h1>
        </div>
    ...
    <form action="{{ route('hadiah.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf

        <div>
            <label class="block font-semibold text-gray-700 mb-1" for="nama_hadiah">Nama Hadiah</label>
            <input type="text" name="nama_hadiah" id="nama_hadiah" placeholder="Nama Hadiah" required
                class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>

        <div>
            <label class="block font-semibold text-gray-700 mb-1" for="image">Gambar Hadiah</label>
            <input type="file" name="image" id="image"
                class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>

        <div>
            <label class="block font-semibold text-gray-700 mb-1" for="points">Point</label>
            <input type="number" name="points" id="points" placeholder="Point" required
                class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>

        <div>
            <label class="block font-semibold text-gray-700 mb-1" for="stok">Stok</label>
        ...
        <div class="flex justify-center gap-4">
            <button type="submit"
                class="p-6 bg-green-500 text-white py-2 rounded-md hover:bg-green-600 transition duration-200">
                Simpan
            </button>
            <a href="{{ route('hadiah.index') }}" class="bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600">
                Kembali  
            </a> 
        </div>
    </form>
