...
<div class="relative flex-1 lg:ml-64 transition-all duration-300 p-6 bg-gray-100 top-0 bottom-0 border-l-8 border-[#ddd] lg:border-l-8 max-lg:border-l-0">
    ...
    <div class="container mx-auto bg-gray-100">
        <div class="flex justify-start mb-6">
            <a href="{{ route('kelola_tantangan.create') }}" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">
                + Tambah Data
            </a>
        </div>
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-200 text-gray-600 uppercase text-xs justify-center">
                            <th class="px-4 py-3">No Tantangan</th>
                    ...
                    <tbody>
                        @forelse ($tantangans as $tantangan)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3">{{ $tantangan->no_tantangan }}</td>
                                <td class="px-4 py-3 font-medium">{{ $tantangan->title }}</td>
                                <td class="px-4 py-3 text-gray-500">{{ Str::limit($tantangan->description, 50) }}</td>
                                <td class="px-4 py-3 text-gray-600">{{ Str::limit($tantangan->details, 30) }}</td>
                                <td class="px-4 py-3">
                                    <a href="{{ $tantangan->video_embed_url }}" target="_blank" class="text-blue-500 hover:underline">Lihat</a>
                                </td>
                                <td class="px-4 py-3">
                                <img src="{{ asset('storage/' . $tantangan->video_thumbnail) }}" alt="thumbnail" class="max-w-[150px] max-h-[150px] rounded shadow">
                                </td>
                                <td class="px-4 py-3">{{ $tantangan->tag }}</td>
                                <td class="px-4 py-3">{{ $tantangan->duration }}</td>
                                <td class="px-4 py-3">{{ $tantangan->poin_tantangan }}</td>
                                <td class="px-4 py-3">{{ $tantangan->tanggal_mulai }}</td>
                                <td class="px-4 py-3">{{ $tantangan->tanggal_selesai }}</td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex justify-center space-x-2">
                                        <a href="{{ route('kelola_tantangan.edit', $tantangan->id) }}" class="bg-blue-500 text-white px-3 py-1 rounded-md">
                                            <i class="ri-edit-2-line"></i>
                                        </a>
                                        <form action="{{ route('kelola_tantangan.destroy', $tantangan->id) }}" method="POST" onsubmit="return confirm('Anda Yakin Untuk Menghapus Tantangan Ini?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="text-center py-10 text-gray-500">Tidak ada tantangan yang tersedia</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
