...
<div class="relative flex-1 lg:ml-64 transition-all duration-300 p-6 bg-gray-100 top-0 bottom-0 border-l-8 border-[#ddd] lg:border-l-8 max-lg:border-l-0">
    ...
    <div class="flex justify-between items-center mb-8"> 
        <div class="mb-8">
            <h1 class="lg:text-2xl font-bold text-gray-800 -mt-5 pt-5 hidden md:block">Kelola Olahraga</h1>
        </div>
        ...
        <div class="container mx-auto">
            <div class="flex justify-between items-center mb-6">
                <a href="{{ route('olahraga.create') }}" class="bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600">
                    Tambah Data
                </a>
            </div>
                ...
                <table class="w-full border-collapse">
                    <thead class="bg-gray-200">
                        <tr>
                            ...
                            <th class="py-2 px-4 text-left">URL Video</th>
                            <th class="py-2 px-4 text-center">Thumbnail</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-t">
                            <td class="py-2 px-4">{{ $olahraga->nama }}</td>
                            <td class="py-2 px-4">{{ $olahraga->deskripsi }}</td>
                            <td class="py-2 px-4">{{ $olahraga->kategori }}</td>
                            <td class="py-2 px-4">{{ $olahraga->durasi }}</td>
                            <td class="py-2 px-4">
                                <a href="{{ $olahraga->url_video }}" target="_blank" class="text-blue-500 hover:underline">
                                    Lihat Video
                                </a>
                            </td>
                            <td class="py-2 px-4 text-center">
                                <img src="{{ asset('storage/' . $olahraga->thumbnails) }}" alt="Thumbnail" class="h-10 mx-auto rounded-lg">
                            </td>
                            <td class="py-4 px-4 text-center space-x-3">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('olahraga.edit', $olahraga->id) }}" class="bg-blue-500 text-white px-2 py-1 rounded-md hover:bg-blue-600 transition duration-300 text-xs inline-flex items-center">
                                        <i class="ri-edit-2-line"></i>
                                    </a>
                                    <form action="{{ route('olahraga.destroy', $olahraga->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirm('Yakin ingin menghapus data ini?');" class="bg-red-500 text-white px-2 py-1 rounded-md hover:bg-red-600 text-xs">
            ...
            <div class="mt-4">
                {{ $olahragas->links() }} <!-- Laravel pagination -->
            </div>
        </div>
    </div>
</div>
